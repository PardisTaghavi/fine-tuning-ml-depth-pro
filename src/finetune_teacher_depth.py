...
def train(model, train_loader, criterion, optimizer, scheduler, device, epoch, args):
    ...
    for i, batch in enumerate(tqdm(train_loader, disable=(dist.get_rank() != 0))):
        images, _, depths = batch  # Use actual depth labels
        ...
        with torch.cuda.amp.autocast():
            ...
            predictions = predictions["depth"].squeeze(1)
            loss_ssi = criterion(predictions, depths)
            loss_grad = grad_loss(predictions, depths)
...
def validate(model, val_loader, criterion, device, epoch, args):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, disable=(dist.get_rank() != 0)):
            ...
            predictions = model.module.infer(images)  # Use model.module for DDP
            predictions = predictions["depth"].squeeze(1)
            loss_ssi = criterion(predictions, gt_depth)
            loss_grad = grad_loss(predictions, gt_depth)
            loss = 3.0 * loss_ssi + 2.0 * loss_grad
            total_loss += loss.item()
